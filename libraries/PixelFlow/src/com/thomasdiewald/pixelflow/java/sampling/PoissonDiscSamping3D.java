...
public abstract class PoissonDiscSamping3D<T extends DwCollisionObject> {
  ...
  public void generatePoissonSampling(float[] bounds, float radius_min, float radius_max, float offset, int new_points){  
    ...
    T sample_new = newInstance(mx, my, mz, pr_new, pr_new+offset);
...
//      int sample_old_idx = stack.pop(rand_ptr);
      ...
      for(int i = 0; i < new_points; i++){
        sample_new = createSample3D(sample_old, radius_min, radius_max, offset);
        addSample(sample_new);
      }
    }
  }
  ...
  public T createSample3D(T sample_old, float radius_min, float radius_max, float offset){
    ...
    float pz_old = sample_old.z();
    
//    float[] xyz = DwSampling.uniformSampleSphere_Halton(halton_idx++);
    float[] xyz = DwSampling.uniformSampleSphere_Random();
    
    float pr_new = radius_min + rand.nextFloat() * (radius_max - radius_min);
    float px_new = px_old + xyz[0] * (pr_old + pr_new + offset*2);
    float py_new = py_old + xyz[1] * (pr_old + pr_new + offset*2);
    float pz_new = pz_old + xyz[2] * (pr_old + pr_new + offset*2);
    return newInstance(px_new, py_new, pz_new, pr_new, pr_new+offset);
  }
